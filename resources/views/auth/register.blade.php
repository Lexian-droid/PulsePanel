<x-layouts.guest>
    ...
    <div class="w-full max-w-sm">
        <div class="mb-8 text-center">
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Sign up to get started with {{ config('app.name') }}.</p>
        </div>
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-800 dark:ring-white/10">
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4">
                ...
                <x-form-field label="Name" required>
                    <x-input type="text" name="name" :value="old('name')" required autofocus placeholder="Your name" />
                </x-form-field>

                <x-form-field label="Email address" required>
                    <x-input type="email" name="email" :value="old('email')" required placeholder="you@example.com" />
                </x-form-field>
                ...
                <x-form-field label="Confirm Password" required>
                    <x-input type="password" name="password_confirmation" required placeholder="••••••••" />
                </x-form-field>
        ...
        <p class="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <a href="{{ route('login') }}" class="font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400">Sign in</a>
        </p>
    </div>
</x-layouts.guest>